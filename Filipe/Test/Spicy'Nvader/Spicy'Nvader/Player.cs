...
namespace Spicy_Nvader
{
    ...
    {     
        ...
        private int _lastDirection;// dèrniere touc

        private bool _movementMode;//Booléeen qui switch entre les 2 différent mode de déplacement
        ...
        {
            UpdateShoot();
            if (!_movementMode && Program.tics%2500 == 0)
            {
                Move(VALUE_OF_MOVEMENT*_lastDirection);
            }
