...
namespace Spicy_Nvader
{
    public class Player
    {
        ...
        private int _playerDirection; // direction du joueur
            ...
            if (Console.KeyAvailable)
            {
                switch (Console.ReadKey(true).Key)//Lis la touche du clavier sur laquelle on appuie
                {
                    case ConsoleKey.RightArrow:                     // 80 - 1  (car 80eme char = pos 79 )    11 / 2 = 5             1    TOT : 73 position max
                        if (_playerPosition <= (Program.WIDTH_OF_WIDOWS - 1) - (PLAYER[4].Length / 2) - Program.MARGIN)//Si la position est plus petite que la valeur max, le joueur se déplace normalement
                        {
                            if (_modeAuto)
                            {
                                _playerDirection = 1;
                            ...
                            {
                                Move(1);//Se déplace (vers la droite)
                            }
                        }
                        else//Si la position du joueur va plus loin que la valeur max, on lui donne la valeur max
                        {
                            ...
                            {
                                Move(0);//La forumule calcule la différence entre la positon actuelle et la valeur max et donne le résultat à la méthode Move
                            }
                        }
                        break;
                    case ConsoleKey.LeftArrow:                   //         11/2 = 5   + 1 TOT : 6 position minimale
                        if (_playerPosition >= PLAYER[4].Length / 2 + Program.MARGIN)
                        {
                            if (_modeAuto)
                            {
                                _playerDirection = 0;
                            ...
                            {
                                Move(-1);//Se déplace (vers la gauche)
                            }
                        }
                        else
                        {
                            ...
                            {
                                Move(0);//Formule qui permet, quelque soit la valeur de mouvement, d'aller le plus à gauche possible au lieu de bloquer avant la marge
                            }
                        }
                        break;
